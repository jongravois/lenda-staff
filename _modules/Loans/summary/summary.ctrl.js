    ...
    angular
        ...
        function SummaryController($rootScope, $scope, AppFactory){
            ...
            $scope.getCropTotal = function(obj) {
                var crop_value = Number(obj.acres) * Number($scope.getCropProdYield(obj) * Number($scope.getCropProdPrice(obj)) * (Number($scope.getCropProdShare(obj)/100)));
                var bk_adj = (obj.bkprice - Number($scope.getCropProdPrice(obj))) * obj.bkqty;
                var harvest_adj = (Number(obj.acres) * Number($scope.getCropProdYield(obj))) * (-1 * obj.var_harvest);
