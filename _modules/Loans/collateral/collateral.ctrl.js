(function(){
    ...
    angular
        ...
        function CollateralsController($rootScope, $scope, $state, AppFactory, LoansFactory){
            ...
            $scope.gridOptsXCol = {
                ...
                columnDefs: [
                    ...
                    },
                    {
                        ...
                        cellTemplate: '<div class="padd black">{{grid.appScope.getXARMCommit(row.entity, grid.appScope.loans)|flexCurrency:0}} </div>',
                        headerCellClass: 'text-center bGreen',
                        enableColumnMenu: false,
                        width: '100'
                    },
                    {
                        ...
                        cellClass: 'text-right',
                        cellTemplate: '<div class="padd black">{{grid.appScope.getXDistCommit(row.entity, grid.appScope.loans)|flexCurrency:0}} </div>',
                        headerCellClass: 'text-center bGreen',
                        enableColumnMenu: false,
                        width: '100'
                    },
                    {
                        ...
                        cellClass: 'text-right',
                        cellTemplate: '<div class="padd black">{{grid.appScope.getXCashFlow(row.entity, grid.appScope.loans)|flexCurrency:0}} </div>',
                        headerCellClass: 'text-center bGreen',
                        enableColumnMenu: false,
                        width: '100'
                    },
                    {
                        ...
                        cellClass: 'text-right',
                        cellTemplate: '<div class="padd black">{{grid.appScope.getXExposure(row.entity, grid.appScope.loans)|flexCurrency:0}} </div>',
                        headerCellClass: 'text-center bGreen',
                        enableColumnMenu: false,
                        width: '100'
                    },
                ...
                data: $scope.loan.xcols
            };

            $scope.msg = {};
            var records = [];
            angular.forEach($scope.loan.xcols, function (rawdata) {
                var record = {};
                record.changedAttrs = {};
            ...
            $scope.gridOptsXCol.onRegisterApi = function(gridApi) {
                //set gridApi on scope
                $scope.$scope = $scope;
                $scope.gridApi = gridApi;
                $scope.xc_hgt = 32 + $scope.loan.xcols.length * 30;
                $scope.xc_wdt = 930;
                $scope.gridApi.gridHeight = $scope.xc_hgt;
                $scope.gridApi.gridWidth = $scope.xc_wdt;
                gridApi.edit.on.afterCellEdit($scope, function(rowEntity, colDef, newValue, oldValue) {
                    $scope.$apply(function(scope) {
                        scope.dirty = true;
                    });
